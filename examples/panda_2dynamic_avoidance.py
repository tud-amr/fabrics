...
# This version was created to demonstrate how to work with multiple dynamic obstacles and test if casadiFunctionwrapper
# was correctly adapted
...
def initalize_environment(render=True):
    ...
    dynamic_obst_dict = {
        "type": "sphere",
        "geometry": {"trajectory": ["-3 + t * 0.1", "-0.6", "0.4"], "radius": 0.1},
    }
...
def parse_runtime_arguments(ob_robot: dict) -> dict:
    x_obsts_dynamic = [ob_robot['FullSensor']['obstacles'][0][0], ob_robot['FullSensor']['obstacles'][2][0]]
    xdot_obsts_dynamic = [ob_robot['FullSensor']['obstacles'][0][1], ob_robot['FullSensor']['obstacles'][2][1]]
    xddot_obsts_dynamic = [ob_robot['FullSensor']['obstacles'][0][2], ob_robot['FullSensor']['obstacles'][2][2]]
    radius_obsts_dynamic = [ob_robot['FullSensor']['obstacles'][0][3], ob_robot['FullSensor']['obstacles'][2][3]]

    collision_links = ['panda_link9', 'panda_link3', 'panda_link4']
    arguments = {}
    arguments["x_obsts_dynamic"] = x_obsts_dynamic
    arguments["xdot_obsts_dynamic"] = xdot_obsts_dynamic
    arguments["xddot_obsts_dynamic"] = xddot_obsts_dynamic
    arguments["radius_obsts_dynamic"] = radius_obsts_dynamic
