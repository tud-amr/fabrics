...
def c2torch(input_file: str, output_file: str, tensor_device='cuda:0'):
    """Translates generated c-code to pytorch tensor function."""
    ...
    python_code_lines = [
        "import torch\n",
        "def casadi_f0_torch(*args):",
        f"    res = torch.zeros((1, {result_shape}, args[0].shape[1]), device='{tensor_device}')",
    ]
    ...
    for line in c_code.split('\n')[2:-2]:
        ...
        if 'casadi_sign' in line:
            line = line.split('=')[0] + ' = torch.sign(' + re.findall('\((.*)\)', line)[0] + ')'
        if 'exp' in line:
            line = line.split('=')[0] + ' = torch.exp(' + re.findall('\((.*)\)', line)[0] + ')'
        if 'tanh' in line:
            line = line.split('=')[0] + ' = torch.tanh(' + re.findall('\((.*)\)', line)[0] + ')'
        if 'fmax' in line:
            line = line.split('=')[0] + ' = torch.fmax(' + re.findall('\((.*)\)', line)[0] + ')'
            # There is a problem with fmax(tensor, float) in torch. Both arguments must be tensors. For now i fix it by hand (making the float a tensor)
        if 'cos' in line:
            line = line.split('=')[0] + ' = torch.cos(' + re.findall('\((.*)\)', line)[0] + ')'
        if 'sin' in line:
            line = line.split('=')[0] + ' = torch.sin(' + re.findall('\((.*)\)', line)[0] + ')'
        if 'casadi_fabs' in line:
            line = line.split('=')[0] + ' = torch.abs(' + re.findall('\((.*)\)', line)[0] + ')'
        line = line.replace('  ', '    ')
    ...
    python_code_lines.append('    return torch.transpose(res[0],0,1)')
