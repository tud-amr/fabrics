...
def main():
    ...
    fk_link8 = forwardKinematics(q_ca[0:10], tip="panda_link8")
    ...
    fk_l_fun = ca.Function("fkl", [q_ca[0:11]], [fk_leftfinger])
    fk_r_fun = ca.Function("fkr", [ca.vertcat(q_ca[0:10], q_ca[11])], [fk_rightfinger])
    fk_8_fun = ca.Function("fk_8", [q_ca[0:10]], [fk_link8])
    # splines
    ...
    dist_r_fun = ca.Function("dist_r", [ca.vertcat(q_ca[0:10], q_ca[11])], [ca.norm_2(x_d + finger_off - fk_rightfinger)])
    dist_l_fun = ca.Function("dist_l", [q_ca[0:11]], [ca.norm_2(x_d - finger_off - fk_leftfinger)])
    ...
    con.addPlane(0.56, fk_leftfinger[2])
    con.addPlane(0.55, fk_rightfinger[2])
    #con.addPlane(0.62, fk_link8[2])
    con.addDamper(n, q_ca)
    con.assembleRootGeometry(m=1.00)
    # controller 2
    con2 = StaticController(n, q_ca, qdot_ca)
    con2.addAttractor(0.0, 1, q_ca[10] + q_ca[11], k=10)
    ...
    q0 = np.array([0.0, 0.0, 0.0, 0.8, -0.7, 0.0, -1.501, 0.0, 1.8675, np.pi/4, 0.02, 0.02])
    t = 0.0
    #ob = env.reset()
    ob = env.reset(q0)
    ...
    for i in range(n_steps):
        ...
        ee_pos = fk_8_fun(ob[0:10])
        dist2obst = np.linalg.norm(ee_pos - obsts[0].x())
        dr = dist_r_fun(np.concatenate((ob[0:10], ob[11:12])))
        dl = dist_l_fun(np.concatenate((ob[0:10], ob[10:11])))
        print("dr : ", dr)
        print("dl : ", dl)
        if (dr + dl) < 0.020:
            print("grasping")
