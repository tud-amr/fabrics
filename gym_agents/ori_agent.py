...
from optFabrics.leaf import Leaf, createAttractor, createCollisionAvoidance, createTimeVariantAttractor, GeometryLeaf, createJointLimits, createRotationAttractor
...
class FabricController():
    def __init__(self, n, x_d, indices, x_obsts, r_obsts, upwards=False):
        ...
        x = ca.SX.sym("x", m)
        xdot = ca.SX.sym("xdot", m)
        fk2 = forwardKinematics(q, x_base, n)[0]
        fk1 = forwardKinematics(q, x_base, n-1)[0]
        lforcing_ori = createRotationAttractor(q, qdot, x, xdot, fk1, fk2)
        ls.append(lforcing_ori)
...
def main():
    ...
    indices = [0, 1]
    con_pre = FabricController(n, x_d, indices, [], [])
