...
from optFabrics.leaf import Leaf, createAttractor, createCollisionAvoidance, GeometryLeaf, createJointLimits
...
class FabricController():
    def __init__(self, n, x_d, indices, x_obsts, r_obsts, redund=False):
        ...
        # redundancy
        if redund:
            q0 = np.array([np.pi/3.0, -np.pi/3.0, 0.0])
            phir = q0 - q
            xr = ca.SX.sym("xr", n)
            xrdot = ca.SX.sym("xrdot", n)
            dm = DiffMap("redundancy", phir, q, qdot, xr, xrdot)
            hr = ca.norm_2(xrdot)**2 * xr
            lamr = 1.0
            ler = ca.norm_2(xrdot)**2 * lamr
            lredundancy = GeometryLeaf("redundancy", dm, ler, hr)
            ls.append(lredundancy)
        # limits
        lim_up = np.array([np.pi, np.pi/2, np.pi/2])
        lim_low = -lim_up
        limitLeaves = createJointLimits(q, qdot, lim_up, lim_low)
        ls += limitLeaves + lcols
        # execution energy
...
def main():
    ...
    con3 = FabricController(3, x_d, indices, [], [], redund=True)
    con4 = FabricController(3, x_d, indices, x_obsts, r_obsts, redund=True)
    ...
    for i in range(len(cons)):
        ...
        for i in range(n_steps):
            ...
            vel = np.linalg.norm(ob[dim: dim * 2])
            if vel < 1e-2:
                print("Stationary...stopping")
                break
            q[:, i] = ob[0:dim]
        qs.append(q)
