...
from nLinkReacher.resources.accController import AccController
...
class FabricController():
    def __init__(self, n, x_d, indices, x_obsts, r_obsts):
        ...
        g = 9.81
        k = 0.0
        self._accController = AccController(n, l, m, g, k)
    ...
    def computeAction(self, z, t):
        ...
        tau = self._accController.control(q, qdot, qddot) 
        return tau
...
def main():
    ...
    for i in range(len(cons)):
        con = cons[i]
        dim = dims[i]
        env = gym.make('nLink-reacher-tor-v0', n=dim, dt=0.01)
        ob = env.reset()
